...
public class Usuario extends _Default {
    ...
    public ArrayList<Usuario> getLista(){
        ...
        try {
            ResultSet resultSet = db.select("SELECT * FROM login");
            if (resultSet != null){
    ...
    public void salvar(){
        String comando = "";
        if (this.getId() == -1){
            comando = String.format("INSERT INTO login (nome, email, telefone) VALUES ('%s','%s','%s');",
                    this.getNome(),this.getEmail(),this.getTelefone());
        }else{
            comando = String.format("UPDATE login SET nome = '%s', email = '%s', telefone = '%s' WHERE id = %d;",
    ...
    public void apagar(){
        String comando = String.format("DELETE FROM login WHERE id = %d;",this.getId());
        DB db = new DB();
        db.execute(comando);
        this._mensagem = db._mensagem;
        this._status = db._status;
